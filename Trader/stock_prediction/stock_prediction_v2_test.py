...
# import dataset
export_dir = 'model/model_keras'
file_name = 'G:\AI Trading\Code\RayTrader_v3\Trader\stock_prediction\data\data_15\ADANIPORTS-EQ.csv'
...
def load_data(stock, seq_len):
    ...
    x_test = data[train_set_size + valid_set_size:-30, :-1, :]
    y_test = data[train_set_size + valid_set_size:-30, -1, :]
...
#
# model.add(tf.keras.layers.LSTM(n_neurons,input_shape=(x_train.shape[1],x_train.shape[2]),activation='linear',
#                                batch_input_shape=(batch_size,x_train.shape[1],x_train.shape[2]),
#                                kernel_initializer='he_normal', return_sequences=True,
#                                stateful=False,kernel_regularizer=tf.keras.regularizers.l2(0.01),
#                 activity_regularizer=tf.keras.regularizers.l1(0.01)))
# model.add(tf.keras.layers.LeakyReLU(alpha=0.3))
# model.add(tf.keras.layers.Dropout(0.25))
# model.add(tf.keras.layers.LSTM(n_neurons,activation='linear',
#                                kernel_initializer='he_normal', return_sequences=True,
#                                stateful=False,kernel_regularizer=tf.keras.regularizers.l2(0.01),
#                 activity_regularizer=tf.keras.regularizers.l1(0.01)))
# model.add(tf.keras.layers.LeakyReLU(alpha=0.3))
...
# model.add(tf.keras.layers.Dense(n_outputs,kernel_regularizer=tf.keras.regularizers.l2(0.01),
#                 activity_regularizer=tf.keras.regularizers.l1(0.01)))
#
# model.compile(loss='mean_squared_error', optimizer='adam')
#
# checkpoint_callback = tf.keras.callbacks.ModelCheckpoint('stocks_pretrained.hdf5'
