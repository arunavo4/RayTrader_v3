...
class Weighted_Unrealized_BS_Env:
    ...
    def __init__(self, data, history_t=90):
        ...
        self.data = data
        self.history_t = history_t
        self.signals = trader_data.get_signals(data, patterns=False)
        self.reset()
    ...
    def reset(self):
        ...
        self.history = [int(0) for _ in range(self.history_t)]
        return np.asarray([self.position_value] + trader_data.get_inputs(self.signals,self.t)) # obs
    ...
    def step(self, act):
        ...
        return np.asarray([self.position_value] + trader_data.get_inputs(self.signals,self.t)), reward, self.done, self.action_record  # obs, reward, done, info
