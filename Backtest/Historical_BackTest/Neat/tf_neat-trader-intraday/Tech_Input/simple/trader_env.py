...
class Weighted_Unrealized_BS_Env:
    ...
    def reset(self):
        ...
        self.history = [int(0) for _ in range(self.history_t)]
        return np.asarray([self.position_value] + self.history) # obs
    ...
    def step(self, act):
        ...
        return np.asarray([self.position_value] + self.history), reward, self.done, self.info  # obs, reward, done, info
