...
class RayTrader:
    ...
    def print_w(self, statement):
        order_dir_path = "G:\AI Trading\Code\RayTrader_v3\Backtest\orders_sma_opt_bo\\" + self.stock_name + ".txt"
            ...
            if new_renko_bars != 0:
                ...
                if new_renko_bars < 4:
                    ...
                    if int(dir) > 0:
                        ...
                        #check if sma has crossed the renko bar and not already bought
                        if low <= sma <= last_renko_price and self.long == False:
                            # check if the stock is already shorted
                            if self.short:
                                self.print_w(
                                    "TimeStamp: {0} \tExit Short LTP: {1} SMA: {2}".format(tick['timestamp'],
                                                                                                    tick['last_price'],
                                                                                                    round(sma, 2)))
                            ...
                            self.print_w("TimeStamp: {0} \tBuy Stock\t LTP: {1} HMA: {2}".format(tick['timestamp'],
                                                                                                          tick['last_price'],
                                                                                                          round(sma, 2)))
                        ...
                        if last_renko_price <= sma <= high and self.short == False:
                            # check if stock already long
                            if self.long:
                                self.print_w(
                                    "TimeStamp: {0} \tExit Long\t LTP: {1} SMA: {2}".format(tick['timestamp'],
                                                                                                     tick['last_price'],
                                                                                                     round(sma, 2)))
                                self.print_w("From: {0} To: {1} Pnl: {2}".format(self.pos, tick['last_price'],
                                                                                 self.get_profit_loss(
                            ...
                            self.print_w(
                                "TimeStamp: {0} \tSell Stock!\t LTP: {1} SMA: {2}".format(tick['timestamp'],
                                                                                                   tick['last_price'],
                                                                                                   round(sma, 2)))
                            self.pos = float(tick['last_price'])
                            self.short = True
