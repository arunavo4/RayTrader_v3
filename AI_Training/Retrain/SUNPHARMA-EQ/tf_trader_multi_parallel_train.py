"""
#############  Multiple Env Trainer in Parallel  #########################
...
def run(n_generations, n_processes):
    ...
    envs = [trader_env.Weighted_Unrealized_BS_Env(train_data) for _ in range(4)]

    evaluator = MultiEnvEvaluator(
        make_net, activate_net, envs=envs, batch_size=batch_size, max_env_steps=max_env_steps
    )
